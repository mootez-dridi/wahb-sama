...
let DemandeFourniture = mongoose.model('reclamations',DemandeFournitureSchema)
...
exports.postNewDemandeFourniture=(nom_demandeur,matricule_demandeur,direction_demandeur,matricule_responsable,besoin,etat)=>{
    return new Promise((reslove,reject)=>{
                mongoose.connect(url,{useNewUrlParser:true,useUnifiedTopology:true}).then(()=>{
                    ...
                    let reclamations = new DemandeFourniture({
...
exports.getDmdFourUser2=(matricule)=>{
    return new Promise((reslove,reject)=>{
        mongoose.connect(url,{useNewUrlParser:true,useUnifiedTopology:true}).then(()=>{      
            ...
            if(doc.length<1){
                mongoose.disconnect()
                reject("Aucune Ticket Fourniture en En att")
            }
            mongoose.disconnect()
            reslove(doc)
        }).catch((err)=>{
